...
export async function hasPermission(userId, permission, context = {}) {
  try {
    console.log('hasPermission called with userId type:', typeof userId, 'value:', userId)
    // Get user role
...
export async function getUserPermissions(userId) {
  try {
    console.log('getUserPermissions called with userId type:', typeof userId, 'value:', userId)
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { role: true }
    });
